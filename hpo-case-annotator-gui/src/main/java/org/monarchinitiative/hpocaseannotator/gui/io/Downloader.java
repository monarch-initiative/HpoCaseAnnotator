package org.monarchinitiative.hpocaseannotator.gui.io;
...
public final class Downloader extends Task<Void> {
    ...
    protected Void call() throws Exception {
        ...
        try (FileOutputStream writer = new FileOutputStream(whereToSave);
            ...
            LOGGER.info(String.format("Done, %d bytes read.", totalBytesRead));
        } catch (MalformedURLException mue) {
            LOGGER.error(String.format("Could not interpret url: %s", url), mue);
        } catch (IOException ioe) {
            LOGGER.error(String.format("IO Exception reading from URL: %s", url), ioe);
        }
        if (isCancelled()) {
            whereToSave.delete();
        }
