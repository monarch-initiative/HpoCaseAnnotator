...
import org.monarchinitiative.hpo_case_annotator.gui.hpotextmining.HPOMiner;
import org.monarchinitiative.hpo_case_annotator.gui.hpotextmining.MinedTerm;
...
public final class Present {
    ...
    /**
     * Header of html defining CSS & JavaScript for the presented text. CSS defines style for tooltips and highlighted
     * text. JavaScript code will allow focus on HPO term in the ontology treeview after clicking on the highlighted
     * text.
     */
    ...
    public void initialize() {
        ...
        miningService.setOnCancelled(e -> {
            LOGGER.info("Cancelling test mining");
            resultHook.accept(new HashSet<>());
        });
        miningService.setOnFailed(e -> {
            PopUps.showWarningDialog("Sorry", "Text mining failed", e.getSource().getMessage());
    ...
    /**
     * Use set of {@link MinedTerm} objects to colorize regions of analyzed (query) text based on which the HPO terms
     * have been identified. Generated HTML content will be presented to the curator in the {@link #webView}.
     *
        ...
        protected Task<Set<MinedTerm>> createTask() {
            return new Task<Set<MinedTerm>>() {
                ...
                protected Set<MinedTerm> call() {
                    ...
                    try {
                        LOGGER.info("Starting text mining using miner {}", miner.toString());
