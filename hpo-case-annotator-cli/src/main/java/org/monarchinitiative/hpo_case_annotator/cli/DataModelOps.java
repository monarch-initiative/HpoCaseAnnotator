...
public class DataModelOps {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DataModelOps.class);
    ...
    public static void main(String[] argv) {
        ArgumentParser parser = ArgumentParsers.newArgumentParser("hca-data-model-ops");
        parser.description("Perform batch operations with data model files.");
        parser.version(getVersion());
        parser.addArgument("--version")
                .help("Show hca-data-model-ops version")
                .action(Arguments.version());
        parser.addArgument("operation")
                .choices("xml2json", "firstxml2xml")
                .type(String.class)
                .help("Operation to be performed on data model files");
        parser.addArgument("sourcedir")
                .type(String.class)
                .help("Path to directory containing data model files that are subject to batch operation");
        parser.addArgument("destdir")
                .type(String.class)
                .help("Path to directory where processed data model files will be stored");
        ...
        DataModelOps dmo = new DataModelOps(args.getString("sourcedir"),
                args.getString("destdir"),
