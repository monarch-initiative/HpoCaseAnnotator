...
//        validator = new GenomicPositionValidator(new File("target/test-classes/testgenome"));
        validator = new GenomicPositionValidator(TestResources.TEST_REF_GENOME);
    }
    ...
    public void validateDiseaseCaseHull() throws Exception {
        try (InputStream is = new FileInputStream(new File("target/test-classes/models/xml/Hull-1994-CFTR.xml"))) {
            DiseaseCaseModel model = XMLModelParser.loadDiseaseCaseModel(is);
